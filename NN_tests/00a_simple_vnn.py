...
def create_dense_nn():
    model = Sequential()
    model.add(Dense(2, input_shape = (3,), activation = activation, 
                    use_bias = False, 
                    kernel_initializer = initializer, 
                    kernel_regularizer = regularizer
                    ))
...
def plot_loss(res_loss, res_val_loss, 
    ...
    plt.legend(['Res_train', 'Res_test', 
                ...
                'Func_tain', 'Func_test'], loc='upper right')

    
    textstr = "Res MSE: {}\nDense MSE: {}\nFunc MSE: {}".format(res_test_mse, 
                                                                dense_test_mse, 
                                                                func_test_mse)
...
restricted_nn = create_restricted_nn()
dense_nn = create_dense_nn()
func_nn = create_functional_model()
...
for size in train_sizes:
    ...
    func_train_mse = func_nn.evaluate(
            [input_df[0], input_df[1], input_df[2]], 
            output_df)
    ...
    func_test_mse = func_nn.evaluate(
            [input_test_df[0], input_test_df[1], input_test_df[2]], 
            output_test_df)
